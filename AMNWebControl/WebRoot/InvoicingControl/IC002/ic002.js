    ...
    $(function ()
	   ...
	   {  
	   	 	  //布局
            ic002layout= $("#ic002layout").ligerLayout({ rightWidth: 400,  allowBottomResize: false, allowLeftResize: false,isRightCollapse:true });
	   		var height = $(".l-layout-center").height();
           	commoninfodivmaster=$("#commoninfodivmaster").ligerGrid({
                columns: [
                { 	name: 'boutercompid', hide:true	},
                { display: '出库单号', 	name: 'bouterbillid', 	width:120,align: 'left'},
                { display: '出库方式', 	name: 'outertype', 		width:120,align: 'left',
           ...
           commoninfodivdetial_pcinser=$("#commoninfodivdetial_pcinser").ligerGrid({
        	...
        	commoninfodivdetial=$("#commoninfodivdetial").ligerGrid({
                columns: [
                { display: '产品编号', 	name: 'outergoodsno',  			width:80,align: 'left' },
                { display: '产品名称', 	name: 'outergoodsname',  		width:150,align: 'left' },
                { display: '单位', 		name: 'outerunit', 				width:50,align: 'left'},
                { display: '库存', 	name: 'curgoodsstock', 				width:50,align: 'right'},
	            { display: '数量', 		name: 'outercount', 			width:50,align: 'right'},
                ...
                onDblClickRow : function (data, rowindex, rowobj)
                {
                 ...
                 toolbar: { items: [
	                		...
	                		{ text: '赠送出库价', click: mangerSendOutGoodsNo, img: contextURL+'/common/ligerui/ligerUI/skins/icons/memeber.gif' }
    				 	 ]}
            });
...
//    
    ...
    { 
     	...
     	var requestUrl ="ic002/loadDgoodsouterss.action"; 
   ...
   function loadGoodprint(request)
   {
    ...
    {
     	var requestUrl ="ic002/loadGoodsOuterInfo.action"; 
   ...
   function loadGoodsOuterInfoMessage(request)
   {
    		...
    		if(responsetext.lsMgoodsouter!=null && responsetext.lsMgoodsouter.length>0)
	   		...
	   		if(returnValue!='')
	   		{
				...
				returnValue=returnValue+']';	
		   		...
		   		cinsertwaremanager= $("#outerwareid").ligerComboBox({  data: JSON.parse(returnValue)
	                , valueFieldID: 'factcouterwareid',label:'请选仓库',labelWidth:200,labelAlign:'center',width:'140',selectBoxHeight:'220'
	            }); 
	        }
	        else
	        {
	        	cinsertwaremanager= $("#outerwareid").ligerComboBox({  data: null , valueFieldID: 'factcouterwareid',label:'选择仓库',labelWidth:200,labelAlign:'center',width:'120',selectBoxHeight:'220'
	            }); 
	        }
    ...
    function loadCurMaster(curMaster)
    {
		try
		{
	    	...
	    	if(checkNull(curMaster.boutercompid)!="001")
	    	{
	    		document.getElementById("revicetype2").style.display="none";
	    		document.getElementById("lbShowText").innerHTML="";
	    		document.getElementById("winsergoodsno_company").readOnly="";
	    	}
	    	...
	    	cinsertwaremanager.selectValue(checkNull(curMaster.outerwareid));
			...
			loadprint();
			if(document.getElementById("outeropationerid").value=="")
			{
				pageWriteState();
				curpagestate=1;
			}
    ...
    try
    {
    	if(postState==1)
		{
   		...
   		postState=1;
    	 showDialogmanager = $.ligerDialog.waitting('正在中,请稍候...');	
    	 curpagestate=3;
    	if(parent.hasFunctionRights( "IC002",  "UR_POST")!=true)
        {
    ...
    function postMessage(request)
    {
     ...
     function addRecord()
     {
     		if(parent.hasFunctionRights( "IC002",  "UR_MODIFY")!=true)
	        {
	        ...
	        else
	        {
				...
				var params = "";			
    	     	var requestUrl ="ic002/add.action"; 
    ...
    {
       	if(parent.hasFunctionRights( "IC002",  "UR_DELETE")!=true)
        {
        	 $.ligerDialog.warn("该用户没有删除权限,请确认!");
        	 return;
        }
        var requestUrl ="ic002/delete.action";
        var params="strCurCompId="+strCurCompId;
        params=params+"&strCurBillId="+strCurBillId;
  ...
  //-------------------------查询信息 ---------------------Start
   ...
   function validateInserper(obj)
   {
   		...
   		var requestUrl ="ic002/validateInserper.action";
        var params="strCurCompId="+document.getElementById("outercompid").value;
   ...
   function validateInserperMessage(request)
     ...
     //---------------------------------------------回车事件
		...
		function validateinserno(obj)
		{
			...
			else
			{
				...
				var requestUrl ="ic002/validateGoodsNo.action";
        		var params="strCurCompId="+document.getElementById("outercompid").value;
        		params=params+"&strCurGoodsId="+obj.value;
        		params=params+"&strWareId="+$("#factcouterwareid").val();
		...
		function validateinserno_company(obj)
		{
			if(document.getElementById("outerstaffid").value=="")
	        {
	        	 $.ligerDialog.warn("请确认出库员工!");
			...
			{
				var requestUrl ="ic002/validateGoodsNoCompany.action";
        		var params="strCurCompId="+document.getElementById("outercompid").value;
        		params=params+"&strCurGoodsId="+obj.value;
        		params=params+"&strWareId="+$("#factcouterwareid").val();
        		params=params+"&strCurDate="+document.getElementById("outerdate").value;
		...
		function validateGoodsNoMessage(request)
	    {
		 	try
			{
		       	...
		       	if(checkNull(strMessage)=="")
		       	{	
		       		if($("#winsergoodsno_company").val()!=""){
		       			document.getElementById("winsergoodsno_company").value=checkNull(responsetext.curGoodsinfo.id.goodsno);
		       		}else{
		...
		function loadGoodsBarCordMessage()
	    {
	       	...
	       	try
			{
				...
				commoninfodivdetial.updateRow(curDetialRecord,{outergoodsno: document.getElementById("winsergoodsno").value
															  ...
															  ,outercount : ForDight(document.getElementById("winsergoodscount").value*1,2)
															  ,outerprice : ForDight(document.getElementById("winsergoodsprice").value*1,2)
															  ,outeramt   : ForDight(document.getElementById("winsergoodsprice").value*1*document.getElementById("winsergoodscount").value*1,2)
	    ...
	    function validateFromBarNo(obj)
	    {
	        ...
	        document.getElementById("wtobarcode").value="";
	        document.getElementById("winsergoodscount").value=0;
	    	var requestUrl ="ic002/validateFromBarNo.action";
        	var params="strCurCompId="+document.getElementById("outercompid").value;
	    ...
	    function validateToBarNo(obj)
	    {
	    	...
	    	var requestUrl ="ic002/validateToBarNo.action";
        	var params="strCurCompId="+document.getElementById("outercompid").value;
		 	...
		 	try
			{
		       	...
		       	{	       		 
		       		...
		       		if(responsetext.goodsbarcount*1>0)
		       		{
						...
						if(document.getElementById("winsergoodscount").value*1>document.getElementById("wcurgoodsstock").value)
						{
							$.ligerDialog.error("出库数量不能超过现有库存量!");
							return;
						}
						else
						{
	...
	function loadGoodsPcinfo(data)
	{
		...
		var requestUrl ="ic002/loadGoodsPc.action";
		var params="strCurCompId="+document.getElementById("outercompid").value;
    ...
    try
    {
       showDialogmanager = $.ligerDialog.waitting('正在复合中,请稍候...');
		var requestUrl ="ic002/handconfirmInfo.action";
    ...
    {
    	var requestUrl ="ic002/handcancelInfo.action";
        var params="strCurCompId="+document.getElementById("outercompid").value;
        params=params+"&strCurBillId="+document.getElementById("outerbillid").value;
    ...
    function searchOrderBill()
    {
    	...
    	var requestUrl ="ic002/searchOrderBill.action";
        var params="strCurCompId="+document.getElementById("outercompid").value;
   ...
   function loadGoodprintshow(request)
   {
 			...
 			lsDgoodsouters=responsetext.lsDgoodsouter;
 			companyinfos=responsetext.companyinfo;
 			var specailurl=contextURL+'/InvoicingControl/IC002/ic002_print.jsp';
	   	...
	   	function loadGoodsOneBarCordMessage(curGoodsinfo,curgoodsstock)
	    {
	       	...
	       	try
			{
				...
				curDetialRecord=commoninfodivdetial.getRow(gridlen-1);
				/*var checkFlag=0;
				var maxlength=commoninfodivdetial.rows.length;
				for (var i=0;i<maxlength*1;i++)
				{
					var row =commoninfodivdetial.getRow(i);
					if(checkNull(row.outergoodsno)==curGoodsinfo.id.goodsno)
					{
						 checkFlag=1;
						 curDetialRecord=row;
						 break;	
				...
				commoninfodivdetial.updateRow(curDetialRecord,{outergoodsno: curGoodsinfo.id.goodsno
															  ...
															  ,outercount : checkNull(curDetialRecord.outercount)*1+1
															  ,outerprice : ForDight(curGoodsinfo.costamtbysale,1)
															  ,outeramt   : ForDight(curGoodsinfo.costamtbysale*(checkNull(curDetialRecord.outercount)*1+1),1)
	    ...
	    function mangerSendOutGoodsNo()
	    {
	    		if(curpagestate==3)
		        {
		        	$.ligerDialog.warn("当前单据不能做赠送单价操作!");
		        	return ;
		        }
	    		var requestUrl ="ic002/mangerSendOutGoodsNo.action";
				var responseMethod="mangerSendOutGoodsNoMessage";
				sendRequestForParams_p(requestUrl,responseMethod,"" ); 	
	    }
	    
	    function mangerSendOutGoodsNoMessage(request)
	    {
		 	try
			{
		       	var responsetext = eval("(" + request.responseText + ")");
		       	...
		       	{	       		 
					...
					{
							var row =commoninfodivdetial.records[rowid];
							commoninfodivdetial.updateRow(row,{outerprice : 0 ,outeramt   : 0});  
					}        
		       	}
		       	else
		       	{
