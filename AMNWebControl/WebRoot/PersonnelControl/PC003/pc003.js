   	...
   	var pc003layout=null;
   	var commoninfodivTradeDate=null;
	var showDialogmanager=null;
	var departmentmanager=null;
	var typechooseData = [{ choose: '0', text: '薪资调整' }, { choose: '1', text: '离职申请'}, { choose: '2', text: '入职申请'}, { choose: '3', text: '重回门店'}, { choose: '1', text: '请假申请'}];
	...
	var comfirmValodateBt=null;
   	$(function ()
   	{
	   try
	   {
	   		  //布局
            pc003layout= $("#pc003layout").ligerLayout({ allowBottomResize: false, allowLeftResize: false });
           	var height = $(".l-layout-center").height();
           	var centerWidth = pc003layout.centerWidth
           ...
           //--------销售条码卡
            commoninfodivTradeDate=$("#commoninfodivTradeDate").ligerGrid({
                columns: [
                { display: '异动类型',   		name: 'bchangetype',  			width:80	,align: 'left', 
	                editor: { type: 'select', data: typechooseData, valueField: 'choose' },
	                render: function (item)
	                {
	                    if (item.bchangetype == 0) return '薪资调整';
	                    else if (item.bchangetype == 1) return '离职申请';
	                    else if (item.bchangetype == 2) return '入职申请';
	                    else if (item.bchangetype == 3) return '重回公司';
	                    else if (item.bchangetype == 4) return '请假申请';
	                    return '';
	                }
	            },
                ...
                {
                   		if (bchangetype == 0) return '薪资调整';
	                    else if (bchangetype == 1) return '离职申请';
	                    else if (bchangetype == 2) return '入职申请';
	                    else if (bchangetype == 3) return '重回公司';
	                    else if (bchangetype == 4) return '请假申请';
	                    return '';
                },
               	onDblClickRow : function (data, rowindex, rowobj)
                {
                	$.ligerDialog.open({ height: 700, url: contextURL+'/PersonnelControl/PC003/staffchange.jsp', width: 550, showMax: false, showToggle: false, showMin: false, isResize: true, title: '人事打印单' });
     			}
	       ...
	       comfirmValodateBt= $("#comfirmValodateButton").ligerButton(
	         {
	             text: '立即生效', width: 160,
		         click: function ()
		         {
    ...
    {
     	var requestUrl ="pc003/loadMaster.action"; 
   ...
   function loadMasterMessage(request)
   {
    ...
    {	
        ...
        var requestUrl ="pc003/loadDataSet.action"; 
	...
	function    initCheckInfo(data)
	{
		...
		if(data.bchangetype==1)
		{
			comfirmValodateBt.setDisabled();
		}
		else
		{
			comfirmValodateBt.setEnabled();
		}
	...
	function checkbill()
	{
	    ...
	    var requestUrl ="pc003/checkBill.action"; 
	...
	function checkinheadbill()
	{
    	...
    	params=params+"&strBillId="+curRecord.bchangebillid;	
    	params=params+"&iChangeType="+curRecord.bchangetype;	
	    var requestUrl ="pc003/checkinheadBill.action"; 
	...
	function comfirmbill()
	{
    	...
    	params=params+"&strBillId="+curRecord.bchangebillid;	
    	params=params+"&iChangeType="+curRecord.bchangetype;	
	    var requestUrl ="pc003/comfrimBill.action"; 
	...
	function comfirmbackbill()
	{
    	...
    	params=params+"&strBillId="+curRecord.bchangebillid;	
    	params=params+"&iChangeType="+curRecord.bchangetype;	
	    var requestUrl ="pc003/comfrimbackBill.action"; 
	...
	function comfirmValodateButton()
	{
		var params = "strCompId="+curRecord.bchangecompid;	
    	params=params+"&strBillId="+curRecord.bchangebillid;	
	    var requestUrl ="pc003/comfirmValodateButton.action"; 
