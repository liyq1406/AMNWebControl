	...
	$(function ()
   	{
   	 try
   	 {
   		...
   		var commoninfodivthirth=$("#commoninfodivthirth").ligerGrid({
	                columns: [
	                { display: '申请店号', 	name: 'bcappcompid', 	width: 50 },
	                { display: '申请单号', 	name: 'bcappcompbillid',  	width: 120 },
	                { display: '申请日期',	name: 'cappdate',  width: 80 },
	                { display: '申请时间', 	name: 'capptime',  width: 60 },
	                { display: '申请人', 		name: 'cappempid',  	width: 60 }	                
	                ],  pageSize:10, 
	                data: {Rows: parent.lsStoreAppBills,Total: parent.lsStoreAppBills.length},      
	                width: '100%',
	                height:'100%',
	                enabledEdit: false,  
	                ...
	                {
	                    loadSelecBillData(data, rowindex, rowobj);
	                }
           			});
            		$("#pageloading").hide();  
    	...
    	function loadSelecBillData(data, rowindex, rowobj)
    	{
       		parent.document.getElementById("cappbillid").value=data.bcappcompbillid;
       		parent.document.getElementById("cappbillid").onchange();
       		parent.showD.hide();
        }
	</script>
		<style type="text/css">
