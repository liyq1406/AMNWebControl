...
@Service
public class IC002Service extends AMN_ModuleService{
	...
	public List loadMasterDataSet(int pageSize, int startRow) {
		return loadMgoodsouter(CommonTool.getCurrDate(),"",pageSize,startRow);
	}
		...
		try
		{
			...
			strSqlHistory=" insert mgoodsstockinfo(changecompid,changetype,changebillno,changedate,changetime,changewareid,changeoption,changestaffid,changeflag)" +
					" values('"+CommonTool.FormatString(curMgoodsouter.getId().getOutercompid())+"','2', " +
							...
							" '"+CommonTool.FormatString(curMgoodsouter.getOuterstaffid())+"',"+CommonTool.FormatInteger(curMgoodsouter.getRevicetype())+") ";
			if(lsDgoodsouter!=null && lsDgoodsouter.size()>0)
			{
				...
				for(int i=0;i<lsDgoodsouter.size();i++)
				{
						 ...
						 if(CommonTool.getLoginInfo("COMPID").equals("001"))
							 ...
							 strSqlBarCode=strSqlBarCode+" update dgoodsbarinfo set barnostate=3,costdate='"+curMgoodsouter.getOuterdate()+"',costbillo='"+curMgoodsouter.getId().getOuterbillid()+"',coststore='"+curMgoodsouter.getOuterstaffid()+"' where goodsbarno between '"+CommonTool.FormatString(lsDgoodsouter.get(i).getFrombarcode())+"' and '"+CommonTool.FormatString(lsDgoodsouter.get(i).getTobarcode())+"' ";
	...
	public boolean handInserBillByOutBill(Mgoodsouter curMgoodsouter,List<Dgoodsouter> lsDgoodsouter)
	{
		try
		{
			 ...
			 record.setId(new MgoodsinsertId(curMgoodsouter.getOuterstaffid(),this.dataTool.loadBillIdByRule(curMgoodsouter.getOuterstaffid(),"mgoodsinsert", "inserbillid", "SP012")));
			 record.setInserdate(CommonTool.getCurrDate());
			 record.setInsertime(CommonTool.getCurrTime());
			 record.setBinsercompid(CommonTool.getLoginInfo("COMPID"));
			 record.setBinserbillid(record.getId().getInserbillid());
			 record.setInsertype(1);
			 record.setInserstaffid(CommonTool.getLoginInfo("USERID"));
			 record.setInserwareid(CommonTool.FormatString(this.getDataTool().loadSysParam(curMgoodsouter.getOuterstaffid(), "SP013")));
			 record.setStoresendbill(curMgoodsouter.getId().getOuterbillid());
			 record.setBillflag(2);
			 record.setInseropationerid(CommonTool.getLoginInfo("USERID"));
			 record.setInseropationdate(CommonTool.getCurrDate());
			 record.setInvalid(0);
			 this.amn_Dao.save(record);
			 ...
			 String strSqlHistory=" insert mgoodsstockinfo(changecompid,changetype,changebillno,changedate,changetime,changewareid,changeoption,changestaffid,changeflag)" +
				" values('"+CommonTool.FormatString(record.getId().getInsercompid())+"','1', " +
						" '"+CommonTool.FormatString(record.getId().getInserbillid())+"'," +
						" '"+CommonTool.FormatString(curMgoodsouter.getOuterdate())+"'," +
						" '"+CommonTool.FormatString(curMgoodsouter.getOutertime())+"'," +
						" '"+CommonTool.FormatString(this.getDataTool().loadSysParam(curMgoodsouter.getOuterstaffid(), "SP013"))+"'," +
						" "+CommonTool.FormatInteger(curMgoodsouter.getOutertype())+"," +
						" '"+CommonTool.FormatString(curMgoodsouter.getOuterstaffid())+"',1) ";
	
			 
			 if(lsDgoodsouter!=null && lsDgoodsouter.size()>0)
			 {
				 List<Dgoodsinsert> lsDgoodsinsert=new ArrayList();
				 Dgoodsinsert insertrecord=null;
				 for(int i=0;i<lsDgoodsouter.size();i++)
				 {
					 insertrecord=new Dgoodsinsert();
					 insertrecord.setId(new DgoodsinsertId(record.getId().getInsercompid(),record.getId().getInserbillid(),lsDgoodsouter.get(i).getId().getOuterseqno()));
					 insertrecord.setInsergoodsno(lsDgoodsouter.get(i).getOutergoodsno());
					 insertrecord.setInserunit(lsDgoodsouter.get(i).getOuterunit());
					 insertrecord.setInsercount(lsDgoodsouter.get(i).getOutercount());
					 insertrecord.setGoodsprice(lsDgoodsouter.get(i).getOuterprice());
					 insertrecord.setGoodsamt(lsDgoodsouter.get(i).getOuteramt());
					 insertrecord.setStandunit(lsDgoodsouter.get(i).getStandunit());
					 insertrecord.setStandprice(lsDgoodsouter.get(i).getStandprice());
					 lsDgoodsinsert.add(insertrecord);
					 strSqlHistory=strSqlHistory+" insert dgoodsstockinfo(changecompid,changetype,changebillno,changeseqno,changegoodsno,standunit,standcount,standprice,producedate,changeunit,changecount,changeprice,changeamt)" +
					 " values('"+CommonTool.FormatString(record.getId().getInsercompid())+"','1', " +
								" '"+CommonTool.FormatString(record.getId().getInserbillid())+"'," +
				 				" "+CommonTool.FormatDouble(lsDgoodsouter.get(i).getId().getOuterseqno())+"," +
	...
	public List<Mgoodsouter> loadMgoodsouter(String strDate,String strBillId,int pageSize, int startRow)
	{
		try
		{
			String strSql="select  mgoodsouter From Mgoodsouter mgoodsouter  " +
					...
					" and (isnull(outerbillid,'')='"+strBillId+"' or '"+strBillId+"'='' ) and isnull(invalid,0)=0 " +
					" and isnull(outerwareid,'') not in ('03','05','06') order by outerdate desc ";
	...
	public Mgoodsouter loadMgoodsouterById(String strCompId,String strBillId)
	{
		try
		{
			String strSql="select  outercompid,outerbillid,outerdate,outertime,outerwareid,outerstaffid " +
					" ,outertype,sendbillid,billflag,outeropationerid,outeropationdate,revicetype " +
					"  From  mgoodsouter,compchaininfo  " +
					" where outercompid=relationcomp and curcomp='"+strCompId+"' and outerbillid='"+strBillId+"'  ";
	...
	//获取明细信息
	 ...
	 public String loadGoodsusetype(String strGoodsNo)
	 {
		 try
		 {
			 String strSql="";
				 strSql=" select goodsusetype from goodsinfo where goodsno = '"+strGoodsNo+"'";
			 AnlyResultSet<String> analysis = new AnlyResultSet<String>()
				{
					public String anlyResultSet(ResultSet rs)
					{
						...
						try
						{
							if(rs != null && rs.next()==true)
							{
								returnValue=rs.getString("goodsusetype");
							}				
						}
						catch(Exception e)
						{
				...
				String strGoodsusetype= (String)this.amn_Dao.executeQuery_ex(strSql,analysis);
				analysis=null;
				return strGoodsusetype;
		 }
		 catch(Exception ex)
		 {
			 ex.printStackTrace();
	 ...
	 public boolean handcancelInfo(String strCompId,String strBillId)
	 {
		 try
		 {
			 ...
			 //删除条码信息
			 if(CommonTool.getLoginInfo("COMPID").equals("001"))
				 strSql=strSql+" update  dgoodsbarinfo set barnostate=1 ,outerdate='',outerbill='',receivestore='' where  outerbill='"+strBillId+"' and barnostate=2 ";
			 else
				 strSql=strSql+" update  dgoodsbarinfo set barnostate=2 ,costdate='',costbillo='',coststore='' where  costbillo='"+strBillId+"' and barnostate=3 ";
	 ...
	 public boolean checkGoodStock(String strCompId,List<Dgoodsouter> lsDgoodsouter)
		{
			boolean bRet=true;
			if(lsDgoodsouter!=null && lsDgoodsouter.size()>0)
			{
				for(Dgoodsouter dgoodsouter:lsDgoodsouter)
				{
					if(checkGoods(strCompId,dgoodsouter.getOutergoodsno(),dgoodsouter.getOutergoodsno(),dgoodsouter.getOutercount().doubleValue())==false)
					{
						bRet=false;
						break;
					}
	 ...
	 public Goodsinfo queryGoodsInfo(String strGoodsId){
		 String hql = "from Goodsinfo where isnull(enablebarcode,1)='0' and goodsno='"+strGoodsId+"'";
		 List<Goodsinfo> lsGoodsinfo=(List<Goodsinfo>)this.amn_Dao.findByHql(hql);
