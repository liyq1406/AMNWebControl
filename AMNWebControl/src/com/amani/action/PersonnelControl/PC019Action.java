...
import com.amani.service.PersonnelControl.PC019Service;
import com.amani.tools.CommonTool;
import com.amani.tools.SystemFinal;
@Scope("prototype")
@ParentPackage("json-default")
@Namespace("/pc019")
public class PC019Action {
	@Autowired
	private PC019Service pc019Service;
	...
	private List<Mstafftargetinfo> lsDataSet;
	private List<Dstafftargetinfo> lsDetialDataSet;
	...
	public String loadDataSet() {
		this.lsDataSet=this.pc019Service.loadDateSetByCompId(strCurCompId, CommonTool.FormatString(strFromDate),billstate);
	...
	public String loadDetialDataSet() {
		this.lsDetialDataSet=this.pc019Service.loadDetialByCompId(strCurCompId,strCurBillId);
		return SystemFinal.LOAD_SUCCESS;
	}
	@Action(value = "loadpc019Excel",  results = { 
			 @Result(name = "load_success", location = "/PersonnelControl/PC019/pc019Excel.jsp"),
			 @Result(name = "load_failure",  location = "/PersonnelControl/PC019/pc019Excel.jsp")	
	}) 
	public String loadpc019Excel() {  
		...
		try
		{
			...
			if(this.lsDataSet!=null&&this.lsDataSet.size()>0)
			{
				for(int i=0;i<this.lsDataSet.size();i++)
				{
					...
					sheet.addCell(new Label(5,i+3,""+this.lsDataSet.get(i).getTargeamt().doubleValue()));
					sheet.addCell(new Label(6,i+3,""+this.lsDataSet.get(i).getSubsidyconditiontext()));
					if(CommonTool.FormatInteger(this.lsDataSet.get(i).getTargeflag())==1)
	...
	{
		this.strMessage="";
		if(this.pc019Service.getDataTool().hasSpecialRights(CommonTool.getLoginInfo("USERID"), "QX016")==false)
		{
			this.strMessage=SystemFinal.NO_OPERATE_RIGHT;
			return SystemFinal.POST_FAILURE;
		}
//		boolean flag=this.pc019Service.handBill(strCompId, strBillId,1);
//		if(flag==false)
//			this.strMessage="操作失败!";
		if(!CommonTool.FormatString(strJsonParam).equals(""))
		{
			this.lsDataSet=this.pc019Service.getDataTool().loadDTOList(strJsonParam, Mstafftargetinfo.class);
			boolean flag=this.pc019Service.handLsBill(lsDataSet,1);
			if(flag==false)
				this.strMessage="操作失败!";
		}
...
//		boolean flag=this.pc019Service.handBill(strCompId, strBillId, 2);
//		if(flag==false)
//			this.strMessage="操作失败!";
		if(!CommonTool.FormatString(strJsonParam).equals(""))
		{
			this.lsDataSet=this.pc019Service.getDataTool().loadDTOList(strJsonParam, Mstafftargetinfo.class);
			boolean flag=this.pc019Service.handLsBill(lsDataSet,2);
			if(flag==false)
				this.strMessage="操作失败!";
		}
		return SystemFinal.POST_SUCCESS;
	...
	{
		this.strMessage="";
		if(this.pc019Service.getDataTool().hasSpecialRights(CommonTool.getLoginInfo("USERID"), "QX025")==false)
		{
			this.strMessage=SystemFinal.NO_OPERATE_RIGHT;
			return SystemFinal.POST_FAILURE;
		}
...
//			this.strMessage="操作失败!";
		...
		{
			this.lsDataSet=this.pc019Service.getDataTool().loadDTOList(strJsonParam, Mstafftargetinfo.class);
			boolean flag=this.pc019Service.handLsBill(lsDataSet,3);
			if(flag==false)
				this.strMessage="操作失败!";
		}
		return SystemFinal.POST_SUCCESS;
	...
	public PC019Service getPc019Service() {
		return pc019Service;
	}
	@JSON(serialize=false)
	public void setPc019Service(PC019Service pc019Service) {
		this.pc019Service = pc019Service;
	}
	public List<Mstafftargetinfo> getLsDataSet() {
		return lsDataSet;
	}
	public void setLsDataSet(List<Mstafftargetinfo> lsDataSet) {
	...
	public List<Dstafftargetinfo> getLsDetialDataSet() {
		return lsDetialDataSet;
	}
	public void setLsDetialDataSet(List<Dstafftargetinfo> lsDetialDataSet) {
		this.lsDetialDataSet = lsDetialDataSet;
	}
