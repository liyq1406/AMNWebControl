...
import com.amani.service.InvoicingControl.IC020Service;
...
@Namespace("/ic020")
public class IC020Action {
	@Autowired
	private IC020Service ic020Service;
	...
	public String query() {
		try{
			dataSet = ic020Service.querySet(strCurCompId, beginDate, endDate,projectKind,status);
		}catch(Exception ex){
			ex.printStackTrace();
		}
	...
	//导出Excel查询
	@Action(value="loadExcel", results={@Result(name ="load_success", location="/InvoicingControl/IC020/ic020Excel.jsp")}) 
	...
	public void createExcel() throws WriteException, IOException{
		try{
			...
			//创建新的一页
			WritableSheet sheet =workbook.createSheet("供应商发货状态", 0);
			//构造表头
			...
			/****创建要显示的内容***/
			//标题
			Label head=new Label(0,0,"供应商发货状态",titleFormate);
			...
			label=new Label(3,2,"订货日期",titleFormate);
			sheet.addCell(label);
			sheet.mergeCells(3, 2, 3, 2);
			label=new Label(4,2,"分单日期",titleFormate);
			sheet.addCell(label);
			sheet.mergeCells(4, 2, 4, 2);
			label=new Label(5,2,"发货日期",titleFormate);
			sheet.addCell(label);
			sheet.mergeCells(5, 2, 5, 2);
			label=new Label(6,2,"收货日期",titleFormate);
			sheet.addCell(label);
			sheet.mergeCells(6, 2, 6, 2);
			label=new Label(7,2,"状态",titleFormate);
			sheet.addCell(label);
			...
			if(dataSet!=null && dataSet.size()>0){
				...
				for (CommonBean bean : dataSet) {
					...
					sheet.addCell(new Label(7, row, bean.getAttr8()));
					row++;
				}
			}
			dataSet=null;
	...
	public IC020Service getIc020Service() {
		return ic020Service;
	}
	@JSON(serialize=false)
	public void setIc020Service(IC020Service ic020Service) {
		this.ic020Service = ic020Service;
	}
