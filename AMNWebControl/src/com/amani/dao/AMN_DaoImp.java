...
public class AMN_DaoImp extends HibernateDaoSupport implements AMN_Dao
{
	@SuppressWarnings("unused")
	@Resource(name="sessionFactory")
    private SessionFactory sessionFactory;

	private static Logger logger = Logger.getLogger(AMN_DaoImp.class.getName());
	 ...
	 * @exception DataAccessException,RuntimeException
	 */
	public Serializable saveByKey(Object model) throws DataAccessException,RuntimeException
	{
	 ...
	 * @exception DataAccessException,RuntimeException
	 */
	public List findByHql(String strSql) throws DataAccessException,RuntimeException
	{
...
//	注意这个函数是自己管理事务的，所以不可以包涵在另外一个事务中
	...
	*/
	...
	//执行标准SQL语句返回是是Result注意这个也是要是被包含在一个事物中的
	public Object executeQuery_ex2(final String strSql,final AnlyResult2Set analysis)
	{
		return (Object)this.getHibernateTemplate().execute(new HibernateCallback(){
			public Object doInHibernate(Session session) throws HibernateException,SQLException
			{
				...
				try{
					...
					Map<String,String> projecetName = new HashMap<String,String>();//动态获取项目名称
					while (rs1 != null && rs1.next()) {
						 ...
						 for (int i = 0; i < metaData.getColumnCount(); i++) {  
						  ...
						  projecetName.put("projectName", columndata);
						}  
					}
					if(statement.getMoreResults()){
						rs2 = statement.getResultSet();
					}
					return analysis.anlyResult2Set(projecetName,rs2);
				} 
				catch(Exception e)
				{
					e.printStackTrace();
