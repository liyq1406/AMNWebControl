alter procedure upg_prepare_ticheng_analysis_after_byday(                    
...
--  where a.staffinid=b.Manageno  and b.compno=@compid    
 ...
 select @ttotalyeji=ttotalyeji,@trealtotalyeji=trealtotalyeji,@ttrhyeji=ttrhyeji,@tbeatyyeji=tbeatyyeji,@tcostcount=tcostcount,@tstaffleavelcount=tstaffleavelcount    
 ...
 --    where  staffinid in('ID00006353','014014302','001016319','ID00001312','ID00004187','ID00007233','ID00000416','014014606','SHYQ00001940','ID00003344') and ISNULL(stafftotalyeji,0)<ISNULL(staffshopyeji,0)    
   ...
   --     group by c.staffinid ) d   on a.staffinid=d.staffinid                      
   --where  a.staffinid='ID00003520' and a.strCompId='048'  
   ...
   --大堂经理(C) 00105  0.001总虚业绩  4    
     update   a set staffshopyeji=(ISNULL(@total_yeji,0)-ISNULL(totalyeji,0))*ISNULL(newyjrate,0)     
   from #staff_work_salary a    
   left join  (select c.staffinid ,totalyeji=SUM(ISNULL(totalyeji,0)) from compclasstraderesult a,staffabsenceinfo b,#staff_work_salary c    
        where staffposition in ('00105','0010502','0010503') and c.staffinid=b.manageno and a.compid=c.strCompId and c.strCompId=b.compid and b.absencedate  between @fromdate and @todate     
        and a.ddate=absencedate    
        group by c.staffinid ) d   on a.staffinid=d.staffinid  
   left join (select manageno,newyjrate from 
   (select row_number() over( PARTITION BY manageno order by max(ISNULL(effectivedate,0)) desc) fid, manageno,newyjrate
   from  #staff_work_salary e,staffhistory f  where e.staffinid=f.Manageno and f.effectivedate<=@todate and ISNULL(newpostion,'') in ('00105','0010502','0010503') 
    group by  manageno,newyjrate,effectivedate ) e where fid=1 )   h on a.staffinid=h.manageno                  
   where  staffposition in ('00105','0010502','0010503') 
  ...
  begin
		...
		where d.csbillid=e.csbillid and d.cscompid=e.cscompid and d.financedate between @fromdate and @todate and d.financedate<='20140831'
		  and d.cscompid=@compid and ISNULL(reservationflag,0)=1 and ((ISNULL(e.csfirstinid,'')=a.staffinid and ISNULL(csfirstreserve,0)=1) or (ISNULL(e.cssecondinid,'')=a.staffinid and ISNULL(cssecondreserve,0)=1) or (ISNULL(e.csthirdinid,'')=a.staffinid and ISNULL(csthirdreserve,0)=1)) ),0)*20                        
		from #staff_work_salary a,staffinfo  b                        
 ...
 update #staff_work_salary set staffshopyeji=isnull(staffshopyeji,0)+ISNULL(@ccount,0)*0.1 where staffposition in('01201','01202')           
   end            update #staff_work_salary set staffshopyeji=(200+isnull(staffshopyeji,0))*(ISNULL(computedays,0)-ISNULL(workdays,0))/ISNULL(computedays,0) where staffposition in('01201','01202')                    
    --调配师补贴    
    set @ccount=0          
  ...
  set staffdebit=isnull((select sum(isnull(rewardamt,0)) from mstaffrewardinfo a,dstaffrewardinfo b where a.entrycompid=b.entrycompid and a.entrybillid=b.entrybillid and handcompid=@compid and isnull(b.billflag,0)=1 and entrydate>=@fromdate and entrydate<
=@todate and handstaffinid=staffinid and entrytype  in ('01','02','05') ),0)                                     
 --代扣                                           
  update #staff_work_salary                                             
  set staffdaikou=isnull((select sum(isnull(rewardamt,0)) from mstaffrewardinfo a,dstaffrewardinfo b where a.entrycompid=b.entrycompid and a.entrybillid=b.entrybillid and handcompid=@compid and isnull(b.billflag,0)=1 and entrydate>=@fromdate and entrydate
<=@todate and handstaffinid=staffinid and entrytype ='12' ),0)                                              
  --迟到早退          
  update #staff_work_salary                                             
 ...
 --  -- end          
  ...
  -- select @ttotalyeji,@trealtotalyeji,@ttrhyeji,@tcostcount,@tstaffleavelcount    
     ...
     where a.staffinid=b.staffinid and b.compid=@compid and b.mmonth=SUBSTRING(@fromdate,1,6)    
     and  isnull(a.workdays,0)<7      
     and isnull(@realtotal_yeji,0)>=ISNULL(@trealtotalyeji,0) 
     and ISNULL(@trealtotalyeji,0)>0    
 ...
 update #staff_work_salary set  needpaysalary=ISNULL(needpaysalary,0)+ isnull(staffsubsidy,0),factpaysalary=isnull(factpaysalary,0)+ isnull(staffsubsidy,0)    
  ...
  delete compute_staff_work_salary_byday where strCompId=@compid and computeday=@fromdate    
  insert compute_staff_work_salary_byday(computeday,strCompId,strCompName,staffno,staffinid,staffname,staffposition,staffpositionname,staffpcid,staffbankaccountno,workdays,computedays,          
         ...
         basestaffamt,staffsocialsource,staffamtchange,stafftargetreward,basestaffpayamt,storesubsidy,yearreward ,markflag,staffcurstate,zerenjinback,zerenjincost)    
  select @fromdate,strCompId,strCompName,staffno,staffinid,staffname,staffposition,staffpositionname,staffpcid,staffbankaccountno,workdays,computedays,          
         stafftotalyeji,staffshopyeji,staffbasesalary,beatysubsidy,oldcustomerreward,leaveldebit,staffsubsidy,staffdebit,staffdaikou,latdebit,staffcost,          
         staffreward,otherdebit,staffsocials,needpaysalary,staydebit,studydebit,salarydebit,factpaysalary,staffmark ,      
