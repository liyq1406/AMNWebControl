alter procedure upg_compute_comp_other_satff_salary_daybyday    
(    
 @compid   varchar(10) ,    
...
begin    
    ...
    update   #allstaff_work_detail_daybyday set arrivaldmonth=datediff(month,arrivaldate,@dateto)
    from  #allstaff_work_detail_daybyday,staffinfo 
    where person_inid=manageno
    
    
    --调动升级 
   
   UPDATE #allstaff_work_detail_daybyday set arrivaldmonth=(select MAX(isnull(effectivedate,'')) from staffhistory c where person_inid=c.manageno and c.changetype in (0,1) and c.oldpostion not in  ('003','006','007','00701','00702')  and isnull(newpostion,'')='003')
   from #allstaff_work_detail_daybyday a,staffhistory b
	...
	(                                    
       ...
       -- if(@compid in ('006','047','033','014','041','046'))
		...
		begin	
			...
			--新6店 5个月以上 发型师 1W-2W(不包含3W) 老客提成比率为30%
			update a set staffticheng=isnull(staffyeji,0)*0.30                                      
			from #allstaff_work_detail_daybyday a ,#emp_yeji_total_resultx b,projectnameinfo with(nolock)                                      
 ...
 update a set cardbigcost=ISNULL((    
  select sum(ISNULL(staffticheng,0)) from #allstaff_work_detail_daybyday b,projectnameinfo c    
   where a.person_inid=b.person_inid and a.ddate=b.srvdate and b.action_id >=7 and b.action_id<=15 and b.code=c.prjno and  c.prjtype<>6 and c.prjpricetype=1 and b.paycode in ('4','17') ),0)    
 from #allstaff_work_analysis_daybyday a    
     
 --销卡小项    
 update a set cardsmallcost=ISNULL((    
  select sum(ISNULL(staffticheng,0)) from #allstaff_work_detail_daybyday b,projectnameinfo c    
   where a.person_inid=b.person_inid and a.ddate=b.srvdate and b.action_id >=7 and b.action_id<=15 and b.code=c.prjno and  c.prjtype<>6 and c.prjpricetype=2 and b.paycode  in ('4','17')  ),0)    
 from #allstaff_work_analysis_daybyday a    
     
 --销卡护理    
 update a set cardhulicost=ISNULL((    
  select sum(ISNULL(staffticheng,0)) from #allstaff_work_detail_daybyday b,projectnameinfo c    
   where a.person_inid=b.person_inid and a.ddate=b.srvdate and b.action_id >=7 and b.action_id<=15 and b.code=c.prjno and  c.prjtype=6  and b.paycode  in ('4','17') ),0)    
 from #allstaff_work_analysis_daybyday a    
