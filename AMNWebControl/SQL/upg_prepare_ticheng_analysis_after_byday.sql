...
--  where a.staffinid=b.Manageno  and b.compno=@compid  
  ...
  from #staff_work_salary a,compchaininfo b  
  where curcomp in ('00101','00105','0010102','0010103') and relationcomp=strCompId  
 ...
 if not exists (select 1 from  compchaininfo where curcomp='0010201' and relationcomp=@compid)  
  ...
  from #staff_work_salary a,( select person_inid,staffyeji=SUM(isnull(staffyeji,0)) from staff_work_salary where compid= @compid and salary_date between '20140401' and '20140410' group by person_inid) b  
  where a.staffinid=b.person_inid and isnull(a.businessflag,0)=1 and ISNULL(staffcurstate,2)=2 and ISNULL(b.staffyeji,0)=0  
   ...
   where staffinid not in ( select person_inid  from staff_work_salary where compid= @compid and salary_date between '20140401' and '20140410' group by person_inid)   
  ...
  from #staff_work_salary a,(select b.staffinid,staffsubsidy=(isnull(b.staffsubsidy,0))*(ISNULL(computedays,0)-ISNULL(workdays,0))/ISNULL(computedays,0) from #staff_work_salary c,#staffsubsidy b where c.staffinid=b.staffinid and ISNULL(computedays,0)>0 ) 
d  
