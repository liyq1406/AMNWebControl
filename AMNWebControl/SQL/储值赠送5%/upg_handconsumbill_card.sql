alter procedure upg_handconsumbill_card          
...
begin           
 ...
 declare @costaccount6	float		--赠送账户    
 ...
 @costaccount6=SUM(case when paymode='17' then isnull(payamt,0) else 0 end )          
 ...
 declare @costaccount6lastamt float  --赠送账户前次余额  
 ...
 begin          
    --生成账户历史          
   select top 1 @costaccount2lastamt=(case when changetype in (0,6,7,8,9,10) then ISNULL(changebeforeamt,0)+isnull(changeamt,0) else ISNULL(changebeforeamt,0)-isnull(changeamt,0) end ) from cardaccountchangehistory where changecardno=@cardno  and changeaccounttype=2 order by chagedate desc,changeseqno desc           
 ...
 if(ISNULL(@costaccount6,0)<>0) --赠送账户          
 begin          
    --生成账户历史          
   select top 1 @costaccount6lastamt=(case when changetype in (0,6,7,8,9,10) then ISNULL(changebeforeamt,0)+isnull(changeamt,0) else ISNULL(changebeforeamt,0)-isnull(changeamt,0) end ) from cardaccountchangehistory where changecardno=@cardno  and changeaccounttype=6 order by chagedate desc,changeseqno desc           
   if(ISNULL(@costaccount6lastamt,0)=0)          
		select @costaccount6lastamt=ISNULL(accountbalance,0) from cardaccount where cardno=@cardno and accounttype=6          
   insert cardaccountchangehistory(changecompid,changecardno,changeaccounttype,changeseqno,changetype,changeamt,changebilltype,changebillno,chagedate,changebeforeamt)          
   values(@compid,@cardno,6,ISNULL(@costaccountseqno,0),2,@costaccount6,'SY',@sendid,@costdate,@costaccount6lastamt)          
   set @costaccountseqno=ISNULL(@costaccountseqno,0)+1          
   --更新账户          
   update cardaccount set accountbalance=ISNULL(accountbalance,0)-ISNULL(@costaccount6,0) where cardno=@cardno and accounttype='6' 
 end
        
 if(ISNULL(@costaccount3,0)<>0) --积分账户          
 begin          
   --生成账户历史          
   select top 1 @costaccount3lastamt=(case when changetype in (0,6,7,8,9,10) then ISNULL(changebeforeamt,0)+isnull(changeamt,0) else ISNULL(changebeforeamt,0)-isnull(changeamt,0) end ) from cardaccountchangehistory where changecardno=@cardno  and changeaccounttype=3 order by chagedate desc, changeseqno desc           
 ...
 begin          
    --生成账户历史          
   select top 1 @costaccount5lastamt=(case when changetype in (0,6,7,8,9,10) then ISNULL(changebeforeamt,0)+isnull(changeamt,0) else ISNULL(changebeforeamt,0)-isnull(changeamt,0) end ) from cardaccountchangehistory where changecardno=@cardno  and changeaccounttype=5 order by chagedate desc,changeseqno desc           
