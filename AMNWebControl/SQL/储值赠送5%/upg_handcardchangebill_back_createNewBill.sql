alter procedure upg_handcardchangebill_back_createNewBill      
@compid   varchar(10), --兑换公司            
@billid   varchar(20), --续卡单号      
@backbillid   varchar(20) --反冲单号          
as            
begin       
 ...
 declare @changedate varchar(10)	--充值日期      
  --获取单据信息      
  select @changetype=changetype,@oldcardno=changebeforcardno,      
         @oldkeepamt=curaccountkeepamt,@olddetamt=curaccountdebtamt,      
         @rechargeamt=changefillamt,@detamt=changdebtamt,@newcardno=changeaftercardno ,@changedate=financedate    
  ...
  -- 充值金额2W上送5%的卡金  账户编号 7
	if(ISNULL(@rechargeamt,0)>=20000 and ISNULL(@SP106,'')='1')
	begin
				if exists(select 1 from cardaccount where cardno=@newcardno and accounttype=6)  
						update cardaccount set accountbalance=isnull(accountbalance,0)+ISNULL(@rechargeamt,0)*0.05*(-1),accountdatefrom=@changedate where cardno=@newcardno and accounttype=6  
				else  
				begin  
					...
					values(@compid,@newcardno,2,ISNULL(@rechargeamt,0)*0.05*(-1),0,@changedate,'','')  
				end  
				...
				values(@compid,@backbillid+'ZS',3,@changedate,@backbillid,@newcardno,@changedate,@rechargeamt,ISNULL(@rechargeamt,0)*0.05*(-1),3)
	end 
  ...
  begin      
    ...
    update sendpointcard set sourcecardno=@oldcardno where   sourcecardno= @newcardno 
  end      
  ...
  insert mcardchangeinfo(changecompid,changebillid,changetype,changedate,changetime,changebeforcardno,changecardfrom,changebeforcardstate,changebeforcardtype,    
    ...
    financedate,operationer,operationdate,backflag,billinsertype)      
    select changecompid,@backbillid,changetype,changedate,changetime,changebeforcardno,changecardfrom,changebeforcardstate,changebeforcardtype,    
    ...
    @curdate,operationer,operationdate,1,billinsertype    
    from mcardchangeinfo where changecompid=@compid and changebillid=@billid    
        
    insert dpayinfo(paycompid,paybillid,paybilltype,payseqno,paymode,payamt)    
    select paycompid,@backbillid,paybilltype,payseqno,paymode,isnull(payamt,0)*(-1) from dpayinfo where paycompid=@compid and paybillid=@billid and paybilltype='ZK'      
