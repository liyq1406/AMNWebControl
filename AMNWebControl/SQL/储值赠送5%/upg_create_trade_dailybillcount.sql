alter procedure upg_create_trade_dailybillcount        
(          
 @compid   varchar(10),          
 @datefrom varchar(8),          
 @dateto   varchar(8)          
...
begin          
 ...
 insert into #temp_result_for_amt(compid,flag,amt)           
 select paycompid,2,isnull(sum(payamt),0) from #dpayinfo_billcount where isnull(paymode,'') in ('4','A','9','17') group by paycompid          
 ...
 insert into #distinct_bill(compid,flag,billid)           
 select distinct paycompid,2,paybillid from #dpayinfo_billcount where isnull(paymode,'') in ('4','A','9','17')           
