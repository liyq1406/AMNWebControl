alter procedure upg_handrechargecardbill_card_back_createNewBill        
...
begin         
 ...
 begin    
   ...
   from sendpointcard where sourcebillid=@billid and sendcompid=@compid    and sendtype=1  and sendpicflag=1    
  ...
  begin
	if exists(select 1 from cardaccount where cardno=@salecardno and accounttype=6)  
			update cardaccount set accountbalance=isnull(accountbalance,0)+ISNULL(@rechargekeepamt,0)*0.05*(-1) where cardno=@salecardno and accounttype=6  
	else  
	begin  
		...
		values(@compid,@salecardno,6,ISNULL(@rechargekeepamt,0)*0.05*(-1),0,@saledate,'','')  
	end  
	...
	values(@compid,@backbillid+'ZS',1,@saledate,@backbillid,@salecardno,@saledate,@rechargekeepamt,ISNULL(@rechargekeepamt,0)*0.05*(-1),3)
  end 
 ...
 update mcardrechargeinfo set backflag=1 where rechargecompid=@compid and rechargebillid=@billid     
  ...
  insert mcardrechargeinfo(rechargecompid,rechargebillid,rechargedate,rechargetime,rechargecardno,rechargecardtype,rechargeaccounttype,rechargetype,    
       ...
       financedate,operationer,operationdate,backbillid,backflag,billinsertype)    
  select rechargecompid,@backbillid,rechargedate,rechargetime,rechargecardno,rechargecardtype,rechargeaccounttype,rechargetype,    
  ...
  @curdate,operationer,operationdate,backbillid,1,billinsertype    
 ...
 select paycompid,@backbillid,paybilltype,payseqno,paymode,isnull(payamt,0)*(-1) from dpayinfo where paycompid=@compid and paybillid=@billid and paybilltype='CZ'      
