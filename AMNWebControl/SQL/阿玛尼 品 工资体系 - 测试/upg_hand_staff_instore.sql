alter procedure upg_hand_staff_instore                  
(                    
  @compid varchar(10),                    
  @datefrom varchar(8),                    
  @dateto varchar(8)                    
...
begin           
  ...
  where compno=@compid and staffno not in (rtrim(@compid)+'300',rtrim(@compid)+'400',rtrim(@compid)+'500',rtrim(@compid)+'600')              
                     
                     
   select  inid,empid=staffno,staffname         
  from #empinfobydate ,staffinfo               
  where isnull(inid,'')<>''  and inid=Manageno and ISNULL(stafftype,0)=0  and staffno  not in (rtrim(@compid)+'300',rtrim(@compid)+'400',rtrim(@compid)+'500',rtrim(@compid)+'600')  
 ...
 (      
  ...
  newcompid,newempid,newdepid,newpostion,newsalary,newyjtype,newyjrate,effectivedate,optionbill      
   ...
   from staffinfo with (NOLOCK)                
   where manageno not in (select manageno from #staffhistory) and compno=@compid  and staffno  not in (rtrim(@compid)+'300',rtrim(@compid)+'400',rtrim(@compid)+'500',rtrim(@compid)+'600')     
  ...
  from staffinfo WITH (nolock) where manageno=@lastemp and @lasttype in ('0','2','4')   and ISNULL(stafftype,0)=0  
   and staffno  not in (rtrim(@compid)+'300',rtrim(@compid)+'400',rtrim(@compid)+'500',rtrim(@compid)+'600')   --haa34c加索引                
  ...
  --                
   ...
   select  inid,empid=staffno,staffname         
   from #empinfobydate ,staffinfo               
   where isnull(inid,'')<>''  and inid=Manageno   and ISNULL(stafftype,0)=0     
   group by inid,staffno,staffname        
   order by      staffno        
