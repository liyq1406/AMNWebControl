...
--  where a.staffinid=b.Manageno  and b.compno=@compid    
  ...
  from #staff_work_salary a,compchaininfo b    
  where curcomp in ('00101','00105') and relationcomp=strCompId    
 ...
 --    where  staffinid in('ID00006353','014014302','001016319','ID00001312','ID00004187','ID00007233','ID00000416','014014606','SHYQ00001940','ID00003344') and ISNULL(stafftotalyeji,0)<ISNULL(staffshopyeji,0)    
   ...
   left join  (select c.staffinid ,realbeautyeji=SUM(ISNULL(realbeautyeji,0)) from compclasstraderesult a,staffabsenceinfo b,#staff_work_salary c    
        ...
        and a.ddate=absencedate    
        group by c.staffinid ) d   on a.staffinid=d.staffinid                      
   where  staffposition='00103'   and ISNULL(tichengmode,1)<>2   
  ...
  --美容师老客补贴 (新战略)
  if exists(select 1 from compchaininfo where curcomp in ('0010104','0010102') and relationcomp=@compid)
  begin
		
	update a                                         
    set oldcustomerreward=isnull((select COUNT(distinct d.csbillid) from mconsumeinfo d with(nolock) ,dconsumeinfo e with(nolock) 
     where d.csbillid=e.csbillid and d.cscompid=e.cscompid and d.financedate between @fromdate and @todate and d.cscompid=@compid and ISNULL(reservationflag,0)=1 and ISNULL(e.csfirstinid,'')=a.staffinid ),0)*20                        
    from #staff_work_salary a,staffinfo  b                        
    where a.staffinid=b.Manageno and b.position in ('004','00401','00402') and ISNULL(curstate,0)=2         
...
<=@todate and handstaffinid=staffinid and entrytype ='12' ),0)                                              
 ...
 --美容顾问+美容经理完成额度/指标额度 *1800 *到勤率 90% 总虚业绩     
  ...
  from #staff_work_salary a,( select person_inid,staffyeji=SUM(isnull(staffyeji,0)) from staff_work_salary where compid= @compid and salary_date between '20140601' and '20140615' group by person_inid) b    
  where a.staffinid=b.person_inid and isnull(a.businessflag,0)=1 and ISNULL(staffcurstate,2)=2 and ISNULL(b.staffyeji,0)=0    
   ...
   where staffinid not in ( select person_inid  from staff_work_salary where compid= @compid and salary_date between '20140601' and '20140615' group by person_inid)     
  ...
  where a.staffinid=b.manageno and b.compno='99999'     
   ...
   --派遣人员门店业绩为0    
   update #staff_work_salary set staffshopyeji=0 ,staffsubsidy=0 where   ISNULL(stafftype,0)=1 and ISNULL(staffno,'') not like '9027%' and staffinid not in ('014014420' , 'ID00003520')     
