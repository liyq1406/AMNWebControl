if exists(select 1 from sysobjects where type='P' and name='upg_hand_personManger_anon')
	drop procedure upg_hand_personManger_anon
go
CREATE procedure upg_hand_personManger_anon  
(
	@compid		varchar(10),                 
	@billNo		varchar(20)   
)
...
begin
	...
	select @oldcompid=appchangecompid, @oldstaffno=changestaffno,@olddepartment=beforedepartment,@oldposition=beforepostation,@oldsalary=beforesalary,
	       ...
	       @newyejiamt=afteryejiamt,@leaveltype=leaveltype,@note=remark,@validDate=validatestartdate,@inlineno=staffmangerno,@billtype=changetype
	...
	if(@billtype = 2)--入职申请               
	begin      
		insert staffhistory(manageno,changetype,oldcompid,oldempid,olddepid,oldpostion,oldsalary,oldyjtype, oldyjrate,oldyjamt,newcompid,newempid,newdepid,newpostion,newsalary,newyjtype,newyjrate,newyjramt,effectivedate)        
		...
		@newdepartment,@newposition,@newsalary,@newyejitype,@newyejirate,@newyejiamt,@curdate)
    ...
    begin          
		...
		values(@inlineno,3,@oldcompid,@oldstaffno,@olddepartment,@oldposition,@oldsalary,@oldyejitype,@oldyejirate,@oldyejiamt,'99999',@inlineno+'LZ',
								@olddepartment,@oldposition,@oldsalary,@oldyejitype,@oldyejirate,@oldyejiamt,@curdate)
